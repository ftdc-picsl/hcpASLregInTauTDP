...
newregdir=/project/ftdc_hcp/aslprep/bbrFlirt/bbrFlirtcbfOutput/sub-${sub}/bbr_out/
...
# check images exist
affmat=${newregdir}/sub-${sub}_asl_to_t1_flirt_plusbbr.mat
affdat=${newregdir}/sub-${sub}_asl_to_t1_flirt_plusbbr.dat
...
for aslfiletag in acq-pcasl_mean_cbf; do
	...
	if [[ ! -f ${movingimg} ]] ; then 
		...
		cmd="${scriptsdir}/asl_to_t1.sh ${sub} ${affdat} ${movingimg} ${freesurferdir} ${aslpostdir} ${aslfiletag}bbrFlirt"
		echo ${cmd}
		${cmd}
		echo "" 
		...
		for lscale in 36 60 125 250; do 
			cmd="${scriptsdir}/clipTheAnnotRemoveThe0s.R ${sub} ${lscale} ${aslfiletag}bbrFlirt ${aslpostdir} ${freesurferdir}"
			echo ${cmd}
			${cmd}
			echo ""
		done
