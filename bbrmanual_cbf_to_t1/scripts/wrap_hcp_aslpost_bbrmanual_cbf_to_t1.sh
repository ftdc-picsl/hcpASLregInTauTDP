...
if [[ $# -lt 1 ]] ; then 
  echo "USAGE: ./wrap_hcp_aslpost_bbrmanual_cbf_to_t1.sh <list of sub ids>  "
...
for i in `cat ${sublist}` ; do 
  ...
  # outputfilename=${aslpostdir}/${sub}/${sub}_${outputfiletag}.nii.gz 
   cmd="bsub -J hcp_aslpost_bbrmanual_cbf_to_t1_${sub} -o ${aslpostdir}/logs/hcp_aslpost_bbrmanual_cbf_to_t1_${sub}.stdout ${morescriptsdir}/hcp_aslpost_bbrmanual_cbf_to_t1.sh ${sub} ${aslprepdir} ${aslprepwrkdir} ${freesurferdir} ${aslpostdir} ${scriptsdir} ${bidsdir}"
  # cmd="${morescriptsdir}/hcp_aslpost_bbrmanual_cbf_to_t1.sh ${sub} ${aslprepdir} ${aslprepwrkdir} ${freesurferdir} ${aslpostdir} ${scriptsdir} ${bidsdir}"
  echo $cmd
  $cmd  
  sleep .12
